...
# to be a count (representing the number of visits this domain received),
# followed by a space, followed by the address. An example of a
...
def subdomainVisits(cpdomains):
    ...
    # Time Complexity -> O(n) -> where n is the lenght of cpdomains
    # Assuming that lenght of cpdomains[i] is fixed
    ...
    for domain in cpdomains:
        # Get the domain count and domain name
        visit_count, domain_name = domain.split()
        visit_count = int(visit_count)

        # Now split this domain again on "."
        sub_domains = domain_name.split(".")

        for i in range(len(sub_domains)):
            x = ".".join(sub_domains[i:])
            if x in my_dict:
                my_dict[x] += visit_count
            else:
                my_dict[x] = visit_count

    result_list = []
    for domain, count in my_dict.items():
        result_list.append(str(count) + " " + domain)
